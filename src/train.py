...
def run_train(model_class: Type[Model],
    ...
    model = model_class(hyperparameters, run_name=run_name,
                        model_save_dir=save_folder, log_save_dir=save_folder)
    if os.path.exists(model.model_save_path):
        ...
        model.load_metadata(
            train_data_dirs, max_files_per_dir=max_files_per_dir, parallelize=parallelize)
    ...
    model_path = model.train(train_data, valid_data,
                             azure_info_path, quiet=quiet, resume=resume)
    return model_path
...
def run(arguments, tag_in_vcs=False) -> None:
    ...
    test_data_dirs = test.expand_data_path(
        arguments['TEST_DATA_PATH'], azure_info_path)

    # default model save location
    if not arguments['SAVE_FOLDER']:
        arguments['SAVE_FOLDER'] = str(
            dir_path.parent/'resources/saved_models/')
    ...
    if testrun:
        compute_evaluation_metrics(
            model_path, arguments, azure_info_path, valid_data_dirs, test_data_dirs, max_files_per_dir)
    else:
        compute_evaluation_metrics(
